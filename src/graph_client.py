...
class GraphClient:
    ...
    async def send_email(
        self, to: list[str], subject: str, body: str, body_type: str = "HTML"
    ) -> dict:
        """Send an email."""
        message = {
            "message": {
    ...
    async def get_events(self, top: int = 10) -> dict:
        """Get calendar events."""
        return await self._request(
            "GET", "/me/events", params={"$top": top, "$orderby": "start/dateTime"}
        )
    ...
    async def get_files(self, folder_path: str = "root") -> dict:
        ...
        endpoint = (
            f"/me/drive/{folder_path}/children"
            if folder_path == "root"
            else f"/me/drive/root:/{folder_path}:/children"
        )
